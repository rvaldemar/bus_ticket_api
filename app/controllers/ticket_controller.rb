class TicketController < ApplicationController
  ...
  def_param_group :tickets do
    ...
    property :status, String, desc: "One of 'processing', 'processed' or 'canceled'. "
  ...
  param :id, String, required: true, desc: 'Trip id, provided by search method. '
  param :payment_details, String, desc: 'Payment details in format that is accepted by the financial department. Format: jsonB String', required: true

  returns array_of: :tickets, desc: "Array of tickets for all subset of routes within the main trip. "
  def buy
    now = Time.now
    @tickets = []
    routes = params[:id].split('_').map { |id| Route.find(id) }
  ...
  param :id, String, required: true, desc: 'Ticket id. '
